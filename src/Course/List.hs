...
seqOptional (x :. xs) = A.liftA2 (:.) x (seqOptional xs)  -- Cheating?

-- For the seqOptional, this could be done by creating a Functor instance
-- of Optional, and by using liftA2, I have done essentially that. 
...
reverse ::
  List a
  -> List a
reverse = foldRight (\n acc -> acc ++ listify n) Nil
  where listify x = (x :. Nil)
...
produce ::
  ...
  -> a
  -> List a
produce f s = s :. letsProduce
  where letsProduce = produce f (f s)
